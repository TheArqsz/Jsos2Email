...
class Jsos:
    ...
    def login(self, is_test: bool = False):
        ...
        If the argument `is_test` isn't passed in,
        connection is checked 10 times.
        If the argument `is_test` is passed in,
        connection is checked one time.
    ...
    def __auth(self, tokens: dict, is_test: bool = False):
        ...
        data = {
            'id1_hf_0': '',
            'oauth_request_url': "{}/oauth/authenticate".format(self.base_oauth_url),  # noqa: E501
            'oauth_consumer_key': tokens['oauth_consumer_key'],
            'oauth_token': tokens['oauth_token'],
            'oauth_locale': tokens['oauth_locale'],
            'oauth_callback_url': "{}/index.php/site/loginAsStudent".format(self.base_jsos_url),  # noqa: E501
            'oauth_symbol': 'EIS',
        ...
        auth_url = self.base_oauth_url \
            + "/oauth/authenticate?0-1.IFormSubmitListener-authenticateForm&" \
            + 'oauth_token=' + tokens['oauth_token'] + '&' \
            + 'oauth_consumer_key=' + tokens['oauth_consumer_key'] + '&' \
            + 'oauth_locale=' + tokens['oauth_locale']
        ...
        for i in range(0, tries):
            ...
            if 'message error' in auth_resp.text:
                raise JsosAuthException(
                    "Login not successful - check username and/or password")
        ...
        raise JsosAuthException(
            "Login not successful after {} tries - exiting".format(tries))
    ...
    def get_messages(self, only_unread: bool = True, max: int = 3) -> list:
        ...
        max : int, optional
            ow many messages to retrieve (default is 3)
    ...
    def __get_message_content(self, url: str) -> str:
        ...
        message_body_string = ''.join([str(x) for x in message_body])
        message_body_string.replace('\n', '')

        # Delete unnecessary headers from text
        message_body_string = message_body_string.replace(
            'Message content', '<b>Original message content:</b>')
        message_body_string = message_body_string.replace(
            'Treść wiadomości', '<b>Originalna treść wiadomości:</b>')
    ...
    def has_unread_messages(self, messages_table=None):
        ...
        If the argument `messages_table` is passed in,
        looks for messages in given htmls.
